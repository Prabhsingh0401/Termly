...
export function Sidebar({ mobileMode = false, onNavigate }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Logo */}
      {!mobileMode && (
        <div className={cn('flex items-center gap-3 px-6 h-16 flex-shrink-0', collapsed && 'justify-center px-0')}>
          {!collapsed && (
            <span className="font-bold text-[25px] tracking-tight text-[var(--text-primary)]">
              Termly
            </span>
          )}
        </div>

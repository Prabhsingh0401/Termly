          ...
          <Card>
            ...
            <dl className="space-y-3 text-sm">
              {[
                ...
                { label: 'Start Date', value: formatDate(contract.startDate ?? contract.start_date) },
                { label: 'End Date', value: formatDate(contract.endDate ?? contract.end_date) },
                { label: 'Notice Period', value: `${contract.noticePeriodDays ?? contract.notice_period_days ?? '—'} days` },
                { label: 'Auto-Renewal', value: contract.autoRenewal ?? contract.auto_renewal ? '✅ Enabled' : '❌ Disabled' },
                { label: 'Governing Law', value: contract.governingLaw ?? contract.governing_law ?? '—' },
                { label: 'Payment Terms', value: contract.paymentTerms ?? contract.payment_terms ?? '—' },
              ].map(({ label, value }) => (
